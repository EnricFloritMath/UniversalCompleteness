...
`IntegerFrequenciesHD.integer_frequencies_higher_dimension_basic` theorem
without importing its proof. [`Solution.lean`](Solution.lean) supplies that
proof from the library. This comparison checks the library theorem, not the
match between the paper-facing [`showcase.lean`](../showcase.lean) and the
paper.
